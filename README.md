...
- **更好的软硬键盘切换**：本平台在物理键盘态下点输入框会弹出软键盘，而软键盘一出现物理键盘工具条
  ...
  - 切到软键盘：搜狗里设的软硬切换热键 / 工具栏最右的「软键盘」按钮 /
    2 秒内进-出-进同一个输入框 / 长按设置里的这一行
  - 切到物理键盘：直接用物理键盘打字，或长按这一行
  - 设置行里有 **[查看切换规则]**，弹窗会列出上面这些动作，并显示你在搜狗里绑的热键
  - 滚动区右下角有悬浮的「刷新状态」按钮，状态不同步时点一下
...
| 选区与分词修复 | 坏宿主（Lexical / WebView 类输入框）里 `Shift`/`Ctrl`+方向键选不了字、选不了词，开启后由模块接管推进选区 | 开 |
| 解除快捷键设置限制 | 搜狗「外接键盘设置」里能录 `Alt`/`Shift`+字母 的组合（放开后冲突与被上层应用抢占需自行判断） | 关 |
| 更好的软硬键盘切换 | 物理键盘态下点输入框不再弹软键盘；状态行显示「物理键盘 / 软键盘」，长按条目可应急切换<br/>切到软键盘：搜狗软硬切换热键 / 工具栏最右的「软键盘」按钮 / 2 秒内进-出-进同一输入框<br/>切到物理键盘：用物理键盘打字 | 关 |
...
「更好的软硬键盘切换」另有状态行（物理键盘 / 软键盘），它的快捷键用的是搜狗「外接键盘设置」里绑的那一个。
进设置页会异步拉一次真实状态：先显示「获取中……」，5 秒没回应就显示「获取超时」；滚动区右下角那个悬浮按钮可以随时手动刷新。
...
├── ShiftArrowRepair.java    # 选区接管：Shift/Ctrl+方向键逐次核对期望焦点，坏宿主由模块推进
├── HotkeyLimitUnlock.java   # 「解除快捷键设置限制」：运行时认出组合键校验器并放行 Alt/Shift
├── HardKeyboardLock.java    # 软硬键盘状态机：iP.a(2) 与 Lta 写点取舍 + jua 切软 + 同框双击判定
